...
onMounted(() => {
  ...
  onValue(dbRef(db, 'routes'), (snapshot) => {
    ...
    if (allRoutes) {
      const routeList = Object.values(allRoutes)
...
<template>
  <div class="min-h-screen bg-gray-950 text-white font-sans flex overflow-hidden">
    ...
    <aside
      class="fixed inset-y-0 left-0 z-50 w-64 bg-black border-r border-gray-800 transition-transform duration-300 flex flex-col lg:static lg:translate-x-0"
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden relative bg-gray-950">
      ...
      <div class="flex-1 overflow-y-auto p-6 lg:p-10 space-y-8">
        ...
        <div v-if="currentTab === 'overview'" class="space-y-8 animate-in fade-in duration-500">
                ...
                <Card class="lg:col-span-2 bg-gray-900 border-gray-800">
                    <CardHeader>
                        <div class="flex items-center justify-between">
                            <div><CardTitle class="text-white text-lg">Live Fill Trends</CardTitle><CardDescription>Real-time History vs AI Prediction</CardDescription></div>
                            <div class="flex gap-4 text-xs"><div class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-emerald-500"></div> Actual</div><div class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-gray-600 border border-white/50"></div> Predicted</div></div>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <div class="relative w-full h-64 mt-4">
                            <div v-if="chartHistory.length === 0" class="flex flex-col items-center justify-center h-full text-gray-500"><Loader2 class="w-8 h-8 animate-spin mb-2" /><span>Reading history...</span></div>
                            <svg v-else viewBox="0 0 100 100" class="w-full h-full overflow-visible" preserveAspectRatio="none">
                                ...
                                <polyline fill="none" stroke="#6b7280" stroke-width="2" stroke-dasharray="4" :points="getPoints('predicted')" vector-effect="non-scaling-stroke" opacity="0.5" />
                                <polyline fill="none" stroke="#10b981" stroke-width="3" :points="getPoints('actual')" vector-effect="non-scaling-stroke" />
                            </svg>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent class="flex-1 flex flex-col items-center justify-center">
                        <div class="relative w-48 h-48">
                           <svg viewBox="0 0 100 100" class="transform -rotate-90">
                              <path :d="getDonutPath(2, activeSensors, statusDist)" fill="#10b981" />
                              <path :d="getDonutPath(1, activeSensors, statusDist)" fill="#f59e0b" />
                              <path :d="getDonutPath(0, activeSensors, statusDist)" fill="#ef4444" />
                           </svg>
                           <div class="absolute inset-0 flex flex-col items-center justify-center">
                              <span class="text-3xl font-bold text-white">{{ activeSensors }}</span>
                              <span class="text-xs text-gray-500">Total Bins</span>
                           </div>
                        </div>
                        <div class="flex gap-4 mt-6 text-xs">
                           <div class="flex items-center gap-1"><div class="w-3 h-3 bg-red-500 rounded-full"></div> Crit</div>
                           <div class="flex items-center gap-1"><div class="w-3 h-3 bg-amber-500 rounded-full"></div> Warn</div>
                           <div class="flex items-center gap-1"><div class="w-3 h-3 bg-emerald-500 rounded-full"></div> OK</div>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <div class="grid lg:grid-cols-3 gap-6">
                <Card class="lg:col-span-2 bg-gray-900 border-gray-800">
                    <CardHeader><CardTitle class="text-white text-lg">Weekly Collection Volume</CardTitle><CardDescription>Total waste collected (Mock Data)</CardDescription></CardHeader>
                    <CardContent>
                       <div class="h-48 w-full flex items-end justify-between gap-2 pt-4">
                          <div v-for="(d, i) in weeklyStats" :key="i" class="flex-1 flex flex-col items-center gap-2 group">
                             <div class="w-full bg-gray-800 rounded-t-sm relative group-hover:bg-gray-700 transition-all h-full flex items-end">
                                <div class="w-full bg-emerald-600/80 rounded-t-sm transition-all duration-500" :style="{ height: d.vol + '%' }"></div>
                             </div>
                             <span class="text-xs text-gray-500 font-mono">{{ d.day }}</span>
                          </div>
                       </div>
                    </CardContent>
                </Card>

                <Card class="bg-gray-900 border-gray-800 flex flex-col h-[300px]">
                    <CardHeader><CardTitle class="text-white text-lg">Live Feed</CardTitle><CardDescription>Real-time updates</CardDescription></CardHeader>
                    <CardContent class="flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        <div v-if="bins.length === 0" class="text-gray-500 text-center py-4">Waiting...</div>
                        <div class="space-y-3">
                            <div v-for="bin in bins" :key="bin.id" class="p-3 rounded-lg border bg-black/40 flex items-center justify-between" :class="bin.fillLevel >= 90 ? 'border-red-500/30' : 'border-gray-800'">
                                <div class="flex items-center gap-3"><div class="w-10 h-10 rounded-lg flex items-center justify-center bg-gray-800"><Wifi class="w-5 h-5" :class="bin.status === 'Critical' ? 'text-red-500' : 'text-emerald-500'" /></div><div><p class="text-sm font-bold text-white">{{ bin.id }}</p><p class="text-xs text-gray-500 truncate w-24">Lat: {{ bin.rawLat }}</p></div></div>
                                <div class="text-right"><p class="text-sm font-bold" :class="bin.fillLevel >= 90 ? 'text-red-400' : 'text-white'">{{ bin.fillLevel }}%</p><span class="text-xs text-gray-500">{{ bin.status }}</span></div>
                            </div>
                        </div>
                    </CardContent>
                </Card>
        ...
        <div v-if="currentTab === 'fleet'" class="animate-in fade-in slide-in-from-right-10 duration-500 space-y-6">
            <Card class="bg-gray-900 border-gray-800">
                ...
                <CardContent class="grid gap-4 md:grid-cols-2">
                    <div class="p-4 bg-black/40 border border-gray-800 rounded-xl flex justify-between items-center">
                        <div><p class="text-white font-bold">Route Optimization</p><p class="text-gray-500 text-sm">Run TSP Algorithm</p></div>
                        <Button @click="handleOptimize" :disabled="isOptimizing" class="bg-emerald-600 hover:bg-emerald-700 text-white min-w-[140px]"><Loader2 v-if="isOptimizing" class="w-4 h-4 mr-2 animate-spin" /><Zap v-else class="w-4 h-4 mr-2" />{{ isOptimizing ? 'Running...' : 'Optimize' }}</Button>
                    </div>
                </CardContent>
            </Card>
        </div>
        ...
        <div v-if="currentTab === 'ml'" class="animate-in fade-in slide-in-from-right-10 duration-500 space-y-6">
            <Card class="bg-gray-900 border-gray-800">
                <CardHeader class="flex flex-row items-center justify-between">
                    <div><CardTitle class="text-white">Predictive Analytics</CardTitle><CardDescription>Forecasts based on historical data</CardDescription></div>
                    <Button @click="handlePredict" :disabled="isPredicting" class="bg-cyan-600 hover:bg-cyan-700 text-white"><Loader2 v-if="isPredicting" class="w-4 h-4 mr-2 animate-spin" /><Brain v-else class="w-4 h-4 mr-2" />{{ isPredicting ? 'Calculating...' : 'Run Inference' }}</Button>
                </CardHeader>
                <CardContent>
                    <div class="rounded-xl border border-gray-800 overflow-hidden">
                        <table class="w-full text-left text-sm">
                            <thead class="bg-gray-800 text-gray-400 uppercase font-medium"><tr><th class="p-4">Bin ID</th><th class="p-4">Current Fill</th><th class="p-4">Fill Rate</th><th class="p-4">Time to Full</th><th class="p-4">Status</th></tr></thead>
                            <tbody class="divide-y divide-gray-800 bg-gray-900/50">
                                <tr v-for="pred in predictions" :key="pred.id" class="hover:bg-white/5 transition">
                                    <td class="p-4 font-bold text-white">{{ pred.id }}</td>
                                    <td class="p-4">{{ pred.currentFill }}%</td>
                                    <td class="p-4 text-gray-400">{{ pred.fillRate }}/hr</td>
                                    <td class="p-4 text-cyan-400 font-bold flex items-center"><Clock class="w-3 h-3 mr-2"/> {{ pred.timeToFull }} hrs</td>
                                    <td class="p-4"><Badge :class="pred.status === 'Critical' ? 'bg-red-500/20 text-red-400' : 'bg-emerald-500/20 text-emerald-400'">{{ pred.status }}</Badge></td>
                                </tr>
                                <tr v-if="predictions.length === 0"><td colspan="5" class="p-8 text-center text-gray-500">No predictions yet. Run inference.</td></tr>
                            </tbody>
                        </table>
                    </div>
                </CardContent>
